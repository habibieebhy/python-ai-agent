...
def _build_session() -> requests.Session:
    ...
    # Robust retry policy (idempotent on 429/500/502/503/504)
    retry = Retry(
        total=4,
        backoff_factor=0.5,
    ...
    s.headers.update({
        ...
        # "Content-Type" auto-set by requests when using json=
    })
    return s

def setup_ai_service() -> Dict[str, Any]:
    """
    Initialize shared requests.Session for OpenRouter.
...
def get_ai_completion(user_message: str) -> Optional[str]:
    """
    Sync call to OpenRouter. Returns assistant content or None.
    Raises on transport/parsing failures.
    """
    global _session
    if _session is None:
...
async def setup_mcp_client(mcp_url: Optional[str] = None) -> Client:
    """
    Initialize and open a global FastMCP client connection.
    Safe to call multiple times; only the first call creates it.
    """
    global _mcp_client
...
async def close_mcp_client():
    """
    Close and clear the global FastMCP client.
    """
    global _mcp_client
    async with _mcp_lock:
        if _mcp_client is not None:
...
# Thin async wrappers mapping 1:1 to your snippet
